...
begin
  ...
  try
    ...
    try
      ...
      UpdateStatus(Format('📦 拷贝完成（约 %.2f MB）', [Copied/1024/1024]));
    ...
    UpdateStatus('💡 测试无误后，请手动删除备份目录，以真正释放C盘空间。');

    ShowChineseMessage('目录迁移完成！' + sLineBreak +
                       '请测试相关程序是否正常运行。' + sLineBreak +
                       '确认无误后可删除备份目录：' + sLineBreak + Backup);
