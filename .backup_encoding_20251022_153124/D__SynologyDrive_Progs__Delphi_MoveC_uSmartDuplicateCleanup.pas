...
begin
  ...
  if Assigned(lblResults) then
  begin
    lblResults.ParentFont := False;
    lblResults.Font.Charset := DEFAULT_CHARSET;
    lblResults.Font.Name := 'Segoe UI';
  end;
  if Assigned(lblFilesFound) then
  begin
    lblFilesFound.ParentFont := False;
    lblFilesFound.Font.Charset := DEFAULT_CHARSET;
    lblFilesFound.Font.Name := 'Segoe UI';
  end;
  if Assigned(lblSpaceSaved) then
  begin
    lblSpaceSaved.ParentFont := False;
    lblSpaceSaved.Font.Charset := DEFAULT_CHARSET;
    lblSpaceSaved.Font.Name := 'Segoe UI';
  end;
  if Assigned(lblSafetyLevel) then
  begin
    lblSafetyLevel.ParentFont := False;
    lblSafetyLevel.Font.Charset := DEFAULT_CHARSET;
    lblSafetyLevel.Font.Name := 'Segoe UI';
  end;
