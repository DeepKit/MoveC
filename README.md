...
### v1.2.0-Beta (2025-12-02)
- ✨ 清理预览功能：清理前可预览所有待清理项目
- ✨ 清理历史记录：查看历史清理记录、导出TXT/JSON报告
- ✨ 预览列表排序：按文件名、大小、类型、修改时间排序
- ✨ 风险等级过滤：按风险显示不同颜色，可隐藏高风险项目
- ✨ 服务控制：Windows更新缓存清理时自动停止/恢复服务
- ✨ 浏览器缓存清理：支持Chrome/Edge/Firefox/IE缓存
- ✨ 系统日志清理：清理Windows日志和调试文件
- ✨ 预取文件清理：清理Windows Prefetch缓存
- ✅ 代码质量重构：拆分巨型类、异常处理优化、内存管理修复
- ✅ 安全增强：系统关键路径黑名单保护，禁止删除System32等
- ✅ UI优化：移除Emoji、统一日志接口、DFM Unicode转义
